...
def gabung_rute(daftar_node):
    """Gabungkan list node jadi teks rute, contoh: ['A','B'] menjadi 'A -> B'"""
    if len(daftar_node) == 0:
        return ""
    teks = daftar_node[0]
    for i in range(1, len(daftar_node)):
        teks = teks + " -> " + daftar_node[i]
    return teks


def gabung_nama(daftar_nama):
    """Gabungkan list nama dengan koma, contoh: ['A','B'] menjadi 'A, B'"""
    if len(daftar_nama) == 0:
        return ""
    teks = daftar_nama[0]
    for i in range(1, len(daftar_nama)):
        teks = teks + ", " + daftar_nama[i]
    return teks
...
def hitung_dijkstra(start_node):    # Menggunakan variabel global yang didefinisikan di luar fungsi
...
def jalankan_simulasi():
    ...
    while sisa_kapasitas > 0:
        ...
        detail_pengiriman.append({
            ...
            "rute": gabung_rute(rute[idx_terpilih]),
    ...
    paket_berhasil = []
    for i in range(1, len(hasil_efisien)):
        paket_berhasil.append(hasil_efisien[i])
...
def main():
    global node, asal, n, jarak, rute, kecepatan, kapasitas, deadline, prioritas, volume
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if jarak_tempuh > 0:
                jarak[i][j] = jarak_tempuh
                jarak[j][i] = jarak_tempuh
    ...
    print(f"Paket yang berhasil dikirim: {gabung_nama(ringkasan['paket_berhasil'])}")
    print(f"Paket yang tidak dapat dikirim: {gabung_nama(ringkasan['paket_gagal'])}")
    return ringkasan
